...
#### 1.Density function ####
dbin_LESSCachedAllSparseBear_v2<- nimbleFunction(run = function( x = double(1)
                                                           , sxy = double(1)
                                                           , sigma = double(0)
   ...
   if(nDetectors<=0)      if(nbDetections == 0){
      if(log == 0) return(1.0)
      else return(0.0)
   }else{
      if(log == 0) return(0.0)
...
registerDistributions(list(
   dbin_LESSCachedAllSparseBear_v2 = list(
      BUGSdist = "dbin_LESSCachedAllSparseBear_v2(sxy, sigma, nbDetections, yDets, 
      detector.xy, trials, detectorIndex, nDetectorsLESS, ResizeFactor, maxNBDets, 
habitatID, maxDist, indicator,
       p0State, detCountries, detRoads,  betaRoads)",
